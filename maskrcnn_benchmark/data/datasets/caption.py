...
class CaptionTSV(TSVYamlDataset):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(yaml_file, None, replace_clean_label)
    ...
    def pack_caption(self, positive_caption, negative_captions, original_tokens_positive):
        if len(negative_captions) == 0:
            return (
                positive_caption,
                original_tokens_positive,
                [(0, len(positive_caption))],
            )
        ...
        return (
            new_caption,
            original_tokens_positive,
            [(start_position, start_position + len(positive_caption))],
        )

    def __get_negative_captions__(self, idx, negative_size=7):
        negative_captions = []
        for i in range(negative_size):
            img, anno, _, scale = super().__getitem__(np.random.choice(len(self)))
    ...
    def __getitem__(self, idx):
        try:
            img, anno, _, scale = super().__getitem__(idx)
            if self.inference_caption:
                ...
                try:
                    ...
                    if self.pack_random_caption_number != 0:
                        ...
                        ) = self.pack_caption(caption, negative_captions, anno["tokens_positive"])
                    else:
