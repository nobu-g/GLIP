...
class VGTSVDataset(TSVYamlDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(yaml_file, cv2_output=cv2_output)
        ...
        if "train" in op.basename(self.linelist_file):
            ...
            raise ValueError(f"Split must be one of [train, test], but get {self.linelist_file}!")
    ...
    def _get_freq_prior(self, must_overlap=False):
        ...
        for ex_ind in range(self.__len__()):
            ...
            if ex_ind % 20 == 0:
                print(f"processing {ex_ind}/{self.__len__()}")
