...
from maskrcnn_benchmark.utils.comm import all_gather, get_rank, get_world_size, is_main_process, synchronize
from maskrcnn_benchmark.utils.flops import profile
from yacs.config import CfgNode, load_cfg  # , _to_dict
...
class EvolutionTrainer:
    ...
    def train(self, train_loader, val_loader):
        ...
        while self.epoch < self.max_epochs:
            ...
            if is_main_process():
                logger.info(f"Epoch {self.epoch + 1} : top {len(self.keep_top_k[self.select_num])} result")
                for i, cand in enumerate(self.keep_top_k[self.select_num]):
                    logger.info(f"     No.{i + 1} {cand} perf = {self.vis_dict[cand]}")
    ...
    def save_candidates(self, cand, template):
        ...
        for i, layer in enumerate(search_layers):
            ...
            layer_setup.append(f"('{name}', {str(setup)[1:-1]})")
        super_cfg.MODEL.BACKBONE.LAYER_SETUP = layer_setup

        cand_cfg = _to_dict(super_cfg)
        del cand_cfg["MODEL"]["BACKBONE"]["LAYER_SEARCH"]
        with open(
            os.path.join(self.cfg.OUTPUT_DIR, os.path.basename(template)).replace(".yaml", f"_cand{cand}.yaml"),
            "w",
        ) as f:
        ...
        cand_keys = [f"layers.{i}.ops.{c}" for i, c in enumerate(paths)]
        ...
        torch.save(
            {"model": cand_weight},
            os.path.join(self.cfg.OUTPUT_DIR, f"init_cand{cand}.pth"),
        )
