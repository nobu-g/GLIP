    ...
    for idx_new, idx_old in enumerate(idxs.tolist()):
        ...
        if model_state_dict[key].shape != loaded_state_dict[key_old].shape:
            if any([k in key_old for k in reshape_keys]):
                ...
                logger.warning(f"Reshaping {key_old} -> {key}. \n")
                ...
                logger.info(
                    "Use coco_in_objects365_inds labelmap for COCO detection because of size mis-match, "
                    "Reshaping {} -> {}. \n".format(key_old, key)
                )
                new_shape = model_state_dict[key].shape
                ...
                logger.info(f"Skip due to size mismatch: {key_old} -> {key}. \n")
                continue
        else:
