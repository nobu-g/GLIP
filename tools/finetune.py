...
def report_freeze_options(cfg):
    ...
    print(
        "Linear Layer (True Prmopt Tuning):",
        cfg.MODEL.DYHEAD.FUSE_CONFIG.ADD_LINEAR_LAYER,
    )
...
def main():
    ...
    for task_id, ft_cfg in enumerate(ft_configs, 1):
        for shuffle_seed in shuffle_seeds:
            ...
            ft_output_dir = output_dir + f"/ft_task_{task_id}"
            ...
            if shuffle_seed is not None:
                ...
                ft_output_dir = ft_output_dir + f"_seed_{shuffle_seed}"

            # Remerge to make sure that the command line arguments are prioritized
            cfg_.merge_from_list(args.opts)
            if "last_checkpoint" in cfg_.MODEL.WEIGHT:
                with open(
                    cfg_.MODEL.WEIGHT.replace("model_last_checkpoint.pth", "last_checkpoint"),
                ) as f:
            ...
            logger.info(f"Loaded fine-tune configuration file {ft_cfg}")
            with open(ft_cfg) as cf:
                config_str = "\n" + cf.read()
                logger.info(config_str)

            output_config_path = os.path.join(ft_output_dir, "config.yml")
            print(f"Saving config into: {output_config_path}")
            # save config here because the data loader will make some changes
            save_config(cfg_, output_config_path)
            logger.info(f"Training {ft_cfg}")
            if custom_shot == 10000:
                if is_main_process():
                    print("Copying pre-training checkpoint")
                    shutil.copy(
                        try_to_find(cfg_.MODEL.WEIGHT),
                        os.path.join(ft_output_dir, "model_best.pth"),
                    )
            else:
                model = train(
                    cfg_,
